...
int wext_get_ssid(const char *ifname, __u8 *ssid)
{
	struct iwreq iwr;
	int ret = 0;
	static int last_connected = -1; /* -1: 初始, 0: 未连接, 1: 已连接; 只在状态翻转时打日志 */
	...
	if (iw_ioctl(ifname, SIOCGIWESSID, &iwr) < 0) {
		...
		ssid[ret] = '\0';
		if (last_connected != 1) {
			DBG_INFO("ioctl[SIOCGIWESSID] connected, ssid = %s", ssid);
			last_connected = 1;
		}
	}
	return ret;
...
int wext_set_passphrase(const char *ifname, const __u8 *passphrase, __u16 passphrase_len)
{
	...
	memset(&iwr, 0, sizeof(iwr));
	iwr.u.data.pointer = (void *) passphrase;
	iwr.u.data.length = passphrase_len;
	iwr.u.data.flags = (passphrase_len != 0);
...
int wext_private_command(const char *ifname, char *cmd, int show_msg)
{
	...
	buf = (char*)rtw_malloc(buf_size);
	if(!buf){
		printf("RTL871X: WEXT: Can't malloc memory");
		return -1;
	}
	memset(buf, 0, buf_size);
	strcpy(buf, cmd);
	...
	if ((ret = iw_ioctl(ifname, SIOCDEVPRIVATE, &iwr)) < 0) {
		printf("RTL871X: ioctl[SIOCDEVPRIVATE] error. ret=%d\n", ret);
	}
	if (show_msg && iwr.u.data.length) {
		if(iwr.u.data.length > buf_size)
			printf("RTL871X: WEXT: Malloc memory is not enough");
		printf("RTL871X: Private Message: %s\n", (char *) iwr.u.data.pointer);
	}
	rtw_free(buf);
	return ret;
}
...
void wext_wlan_indicate(unsigned int cmd, union iwreq_data *wrqu, char *extra)
{
	...
	{
		case SIOCGIWAP:
			if((u8)wrqu->ap_addr.sa_family == ARPHRD_ETHER)
			{
...
#if (defined(CONFIG_ENABLE_P2P) && CONFIG_ENABLE_P2P) || CONFIG_AP_MODE
...
int wext_get_drv_ability(const char *ifname, __u32 *ability)
{
	...
	snprintf(buf, 33, "get_drv_ability %p", ability);
	ret = wext_private_command(ifname, buf, 0);
